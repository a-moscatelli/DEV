...
public class HelloWorld extends HttpServlet {
    ...
    private synchronized void start_en(String kb_) throws DatalogParseException,DatalogValidationException {
        //assert rd != null;
        // kb = kb_; too early!
        System.out.println("INFO start_en() - new kb, size of text = " + kb_.length());
...
// edge(a, b). edge(b, c). edge(c, d). edge(d, c). tc(X, Y) :- edge(X, Y). tc(X, Y) :- tc(X, Z), tc(Z, Y).
		...
		if(uc.equals("learn")) {  // ?uc=learn&kb=........
					rc = 400;
					try {
